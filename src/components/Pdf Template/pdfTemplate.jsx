                        ...
                        <div className="grid grid-cols-3 gap-3 pt-8">
                            {Object.keys(formData).map((key) => (
                                <div className="mb-4" key={key}>
                                    <label className="text-xl font-semibold">{key.replace(/([A-Z])/g, ' $1').toUpperCase()}</label>
                                    <input
                                        type={key.includes('Email') ? 'email' : key.includes('Date') ? 'date' : 'text'}
                                        name={key}
                                        value={formData[key]}
                                        onChange={handleChange}
                                        className="my-2 p-2 border-style w-full focus:ring-0"
                                    />
                                </div>
                            ))}
                        </div>
                                ...
                                <table className="w-full border-collapse border shadow-lg">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td className="border"><input type="date" name="invoiceDate" className="p-2 border-none w-full focus:ring-0" value={formData.invoiceDate} onChange={handleChange} /></td>
                                        </tr>
                                        <tr style={{ backgroundColor: '#fee2e2' }}>
                                            <td className="border  px-4 py-2 font-semibold">Expiry</td>
                                            <td className="border"><input type="date" name="expiryDate" className="p-2 border-none w-full focus:ring-0 bg-[#fee2e2]" value={formData.expiryDate} onChange={handleChange} /></td>
                                        </tr>
                                    </tbody>
                                </table>
                                <div className="mt-12">
